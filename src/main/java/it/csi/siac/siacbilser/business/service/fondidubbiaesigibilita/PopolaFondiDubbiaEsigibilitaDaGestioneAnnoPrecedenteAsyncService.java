...
public class PopolaFondiDubbiaEsigibilitaDaGestioneAnnoPrecedenteAsyncService extends AsyncBaseService<PopolaFondiDubbiaEsigibilitaDaGestioneAnnoPrecedente,
		//response del servizio sincrono
		PopolaFondiDubbiaEsigibilitaDaGestioneAnnoPrecedenteResponse,
		//request del servizio asincrono
		AsyncServiceRequestWrapper<PopolaFondiDubbiaEsigibilitaDaGestioneAnnoPrecedente>,
		//gestore della risposta del servizio sincrono
		PopolaFondiDubbiaEsigibilitaDaGestioneAnnoPrecedenteAsyncResponseHandler,
		//servizio sincrono
		PopolaFondiDubbiaEsigibilitaDaGestioneAnnoPrecedenteService> {
	...
	@Override
	@Transactional
	public AsyncServiceResponse executeService(AsyncServiceRequestWrapper<PopolaFondiDubbiaEsigibilitaDaGestioneAnnoPrecedente> serviceRequest) {
		//eseguo il servizio
	...
	protected void preStartService() {
		...
		if(esisteElaborazioneAttiva){
			//e' in corso un'elaborazione concorrente, non posso continuare
	...
	private AttributiBilancio ottieniAttributiBilancio(Bilancio bilancio) {
		...
		if(attributiBilancio == null) {
			...
			throw new BusinessException(ErroreCore.ENTITA_INESISTENTE.getErrore("popolamento fondi a dubbia e difficile esazione da gestione anno precedente",
	...
	protected void startService() {
		...
		try{
			elaborazioniManager.startElaborazione(PopolaFondiDubbiaEsigibilitaDaGestioneAnnoPrecedenteAsyncService.class, "bilancio.uid:" + originalRequest.getBilancio().getUid());
		} catch (ElaborazioneAttivaException eae){
			//esiste un'elaborazione concorrente: non posso continuare
			String msg = "L'elaborazione fundi dubbia esigibilita' da gestione anno precedente per il bilancio e' gia' in corso. [uid: " + originalRequest.getBilancio().getUid() + "]";
