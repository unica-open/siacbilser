...
public abstract class StampaLiquidazioneIvaBaseVenditeDataCompiler extends StampaLiquidazioneIvaBaseDataCompiler<SubdocumentoIvaEntrata> {
	...
	protected void popolaSezione1() {
		 ...
		 */
		List<StampaLiquidazioneIvaDatoIva> listaRighe = new ArrayList<StampaLiquidazioneIvaDatoIva>();
		Map<String, StampaLiquidazioneIvaDatoIva> mappaRighe = new HashMap<String, StampaLiquidazioneIvaDatoIva>();
		
		for(SubdocumentoIvaEntrata si : listaSubdocumentoIva) {
			...
			for(AliquotaSubdocumentoIva asi : si.getListaAliquotaSubdocumentoIva()) {
				...
				if(mappaRighe.containsKey(keyRegistroAliquotaPeriodo)){
					StampaLiquidazioneIvaDatoIva riga = mappaRighe.get(keyRegistroAliquotaPeriodo);
					riga.setImponibile(riga.getImponibile().add(asi.getImponibile()));
					riga.setImposta(riga.getImposta().add(asi.getImposta()));
				}else{
					StampaLiquidazioneIvaDatoIva riga = new StampaLiquidazioneIvaDatoIva();
		...
		for(StampaLiquidazioneIvaDatoIva riga: mappaRighe.values()){ 
			if(!isRigaToSkip(riga.getImponibile(), riga.getImposta())){
				listaRighe.add(riga);
			}
		}
		pagina.setListaDatiIva(listaRighe);
