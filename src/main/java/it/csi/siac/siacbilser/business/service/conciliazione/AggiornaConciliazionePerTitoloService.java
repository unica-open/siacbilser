...
public class AggiornaConciliazionePerTitoloService extends CheckedAccountBaseService<AggiornaConciliazionePerTitolo, AggiornaConciliazionePerTitoloResponse> {
	...
	@Override
	@Transactional
	public AggiornaConciliazionePerTitoloResponse executeService(AggiornaConciliazionePerTitolo serviceRequest) {
	...
	protected void checkServiceParam() throws ServiceParamError {
		...
		checkCondition(entitaValorizzata(conciliazionePerTitolo.getMacroaggregato()) ^ entitaValorizzata(conciliazionePerTitolo.getCategoriaTipologiaTitolo()),
	...
	protected void execute() {
		...
		// 4. Aggiornamento
		conciliazioneDad.aggiornaConciliazionePerTitolo(conciliazionePerTitolo);
	...
	private void checkContoGSA() {
		...
		conciliazionePerTitolo.getConto().setCodice(conto.getCodice());
	}
	...
	private void checkNonGiaCollegato() {
		...
		Long count = conciliazioneDad.countConciliazioniTitoloByContoAndClassificatoreAndClasse(conciliazionePerTitolo.getConto(), classificatoreGerarchico,
				conciliazionePerTitolo.getClasseDiConciliazione(), conciliazionePerTitolo.getUid());
		log.debug(methodName, "Numero conciliazioni per conto [" + conciliazionePerTitolo.getConto().getUid() + "], classificatore [" + classificatoreGerarchico.getUid() + "], classe ["
				+ conciliazionePerTitolo.getClasseDiConciliazione() + "] distinte da quella da aggiornare [" + conciliazionePerTitolo.getUid() + "]: " + count);
		if(count != null && count.longValue() > 0L) {
			throw new BusinessException(ErroreCore.OPERAZIONE_NON_CONSENTITA.getErrore("il conto " + conciliazionePerTitolo.getConto().getUid() + " e' gia' associato alla classe di conciliazione"));
		}
	}
}
