...
/**
 * The Class MovimentoEPMovimentoDettaglioContoConverter.
 */
@Component
public class MovimentoEPMovimentoDettaglioContoConverter extends ExtendedDozerConverter<MovimentoEP, SiacTMovEp> {
	

	/**
	 * Instantiates a new subdocumento Entrata subdocumento iva converter.
	 */
	public MovimentoEPMovimentoDettaglioContoConverter() {
	...
	public MovimentoEP convertFrom(SiacTMovEp src, MovimentoEP dest) {
		...
		for(SiacTMovEpDet siacTMovEpDet : src.getSiacTMovEpDets()){
			if(siacTMovEpDet.getDataCancellazione() == null){
				MovimentoDettaglio movimentoDettaglio = map(siacTMovEpDet, MovimentoDettaglio.class, GenMapId.SiacTMovEpDet_MovimentoDettaglio_DettaglioConto);
