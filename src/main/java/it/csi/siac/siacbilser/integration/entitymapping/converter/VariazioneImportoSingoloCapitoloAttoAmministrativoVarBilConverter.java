 ...
 */
@Component
public class VariazioneImportoSingoloCapitoloAttoAmministrativoVarBilConverter extends ExtendedDozerConverter<VariazioneImportoSingoloCapitolo, SiacTVariazione> {

	/** Costruttore vuoto */
	public VariazioneImportoSingoloCapitoloAttoAmministrativoVarBilConverter() {
	...
	public VariazioneImportoSingoloCapitolo convertFrom(SiacTVariazione src, VariazioneImportoSingoloCapitolo dest) {
		...
		for (SiacRVariazioneStato siacRVariazioneStato : src.getSiacRVariazioneStatos()) {
			...
			if(siacRVariazioneStato.getDataCancellazione() == null && siacRVariazioneStato.getSiacTAttoAmm() != null){
				...
				SiacTAttoAmm siacTAttoAmm = siacRVariazioneStato.getSiacTAttoAmmVarbil();
				log.debug(methodName, "siacRVariazioneStato.siacTAttoAmmVarBil " +  (siacTAttoAmm!= null ?  "uid = " + siacTAttoAmm.getUid() : " null"));
				...
				dest.setAttoAmministrativoVariazioneDiBilancio(attoAmministrativo);
			}
		}
		return dest;
	}
	...
	public SiacTVariazione convertTo(VariazioneImportoSingoloCapitolo src, SiacTVariazione dest) {
		...
		siacTAttoAmm.setAttoammId(src.getAttoAmministrativoVariazioneDiBilancio().getUid());
