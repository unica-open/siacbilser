 ...
 */
@Component
public class MovimentoEPMovimentoDettaglioModelDetailConverter extends ExtendedDozerConverter<MovimentoEP, SiacTMovEp> {
	

	/**
	 * Instantiates a new subdocumento Entrata subdocumento iva converter.
	 */
	public MovimentoEPMovimentoDettaglioModelDetailConverter() {
	...
	public MovimentoEP convertFrom(SiacTMovEp src, MovimentoEP dest) {
		...
		for(SiacTMovEpDet siacTMovEpDet : src.getSiacTMovEpDets()){
			if(siacTMovEpDet.getDataCancellazione() == null){
				MovimentoDettaglio movimentoDettaglio = mapNotNull(siacTMovEpDet, MovimentoDettaglio.class, 
						GenMapId.SiacTMovEpDet_MovimentoDettaglio_ModelDetail, 
						Converters.byModelDetails(Utility.MDTL.byModelDetailClass(MovimentoDettaglioModelDetail.class)));
