...
import it.csi.siac.siacbilser.integration.dad.mapper.attoamministrativo.SiacDAttoAmmTipoTipoAttoMapper;
...
/**
 * The Class AttoAmministrativoDad.
 */
@Component
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
@Transactional
public class AttoAmministrativoDad extends ExtendedBaseDadImpl {
	...
	@Autowired
	private SiacTAttoAmmAttoAmministrativoMapper siacTAttoAmmAttoAmministrativoMapper;

	@Autowired
	private SiacDAttoAmmTipoTipoAttoMapper siacDAttoAmmTipoTipoAttoMapper;
...
public List<AttoAmministrativo> ricercaProvvedimenti(RicercaAtti ricerca) {
		
		List<AttoAmministrativo> elencoAttiLeggeTrovati = new ArrayList<AttoAmministrativo>();
		
		List<SiacTAttoAmm> result = siacTAttoAmmDao.ricercaProvvedimento(siacTEnteProprietario,
				mapToString(ricerca.getAnnoAtto(), ""),
				ricerca.getOggetto(),
				ricerca.getNumeroAtto(),
				ricerca.getNote(),
				mapToUidIfNotZero(ricerca.getTipoAtto()),
				ricerca.getUid(),
				mapToUidIfNotZero(ricerca.getStrutturaAmministrativoContabile()),
				ricerca.getStatoOperativo(),
				ricerca.isConDocumentoAssociato(),
				//SIAC-6929
				ricerca.getBloccoRagioneria(),
				ricerca.getProvenienza() 
				);
...
//		attoAmministrativo = map(attoDaSalvare, AttoAmministrativo.class, AttMapId.SiacTAttoAmm);
	...
	public AttoAmministrativo update(AttoAmministrativo attoAmministrativo,
			StrutturaAmministrativoContabile strutturaAmministrativoContabile,
			TipoAtto tipoAtto, String codiceInc) throws AttoNonTrovatoException {
		...
		// SIAC-7897
		attoDaAggiornare.setAttoammAnno(String.valueOf(attoAmministrativo.getAnno())); 
		...
		if (StringUtils.isNotBlank(codiceInc)) {
			attoDaAggiornare.setLoginOperazione(codiceInc);
		}
	...
	//SIAC-8729
	public void aggiornaParereFinanziarioImpegniEAccertamentiCollegati(AttoAmministrativo attoAmministrativo, Boolean isEsecutivo) {
		final String methodName = "aggiornaParereFinanziarioImpegniEAccertamentiCollegati";
		siacTAttoAmmDao.aggiornaParereFinanziarioMovgestTs(attoAmministrativo.getUid(), Boolean.TRUE.equals(isEsecutivo),  loginOperazione);
//		if(uidsRegistrazioni == null) {
//			log.debug(methodName, "Returning null list from dao...");
//			return;
//		}
...
//		return atto.getSiacRAttoAmmStatos().get(0).getSiacDAttoAmmStato().getAttoammStatoCode().equals(StatoEntita.CANCELLATO.name());
	 ...
	 */
	@Deprecated
	public TipoAtto findTipoAttoByUid_DEPRECATED(Integer uidTipoAtto) {
	...
	public TipoAtto findTipoAttoByUid(Integer uidTipoAtto) {
		return siacDAttoAmmTipoTipoAttoMapper.map(siacDAttoAmmTipoRepository.findOne(uidTipoAtto));
	}
	
	public TipoAtto findTipoAttoByKey(TipoAtto tipoAtto) {
		
		if (tipoAtto == null) {
			return null;
		}
		
		if (tipoAtto.getUid()!=0) {
			return findTipoAttoByUid(tipoAtto.getUid());
		}
		
		return siacDAttoAmmTipoTipoAttoMapper.map(siacDAttoAmmTipoRepository.ricercaTipoAttoAmm(ente.getUid(), tipoAtto.getCodice()));
	}

	/**
	 * Cerca un atto amministrativo a partire dall'anno,numero e tipo e restituisce un oggetto completo
	 ...
	 * @return
	 */
	public AttoAmministrativo findAttoAmministrativoByLogicKey(AttoAmministrativo attoAmministrativo){
		if (attoAmministrativo.getTipoAtto().getUid()==0) {
			TipoAtto tipoAtto = findTipoAttoByKey(attoAmministrativo.getTipoAtto());
			if (tipoAtto==null) {
				throw new BusinessException(ErroreCore.ENTITA_NON_TROVATA.getErrore("tipoAtto", attoAmministrativo.getTipoAtto().getCodice()));
			}
			attoAmministrativo.setTipoAtto(tipoAtto);
		}
		
		SiacTAttoAmm siacTAttoAmm = siacTAttoAmmRepository.ricercaAttoByAnnoNumeroTipoAndSAC( ente.getUid(),
				String.valueOf(attoAmministrativo.getAnno()), 
				attoAmministrativo.getNumero(),
				attoAmministrativo.getTipoAtto().getUid(),
				attoAmministrativo.getStrutturaAmmContabile() != null ? attoAmministrativo.getStrutturaAmmContabile().getUid(): null);
		...
		return siacTAttoAmmAttoAmministrativoMapper.map(siacTAttoAmm);
	 ...
	 */
	public TipoAtto findTipoAtto(SiacDAttoAmmTipoEnum siacDAttoAmmTipoEnum, Ente ente) {
		SiacDAttoAmmTipo siacDAttoAmmTipo = eef.getEntity(siacDAttoAmmTipoEnum, ente.getUid());
		return mapNotNull(siacDAttoAmmTipo, TipoAtto.class, BilMapId.SiacDAttoAmmTipo_TipoAtto);
	}
	
	public TipoAtto findTipoAtto(String codiceTipoAtto, Ente ente) {
		SiacDAttoAmmTipo siacDAttoAmmTipo = siacDAttoAmmTipoRepository.ricercaTipoAttoAmm(ente.getUid(), codiceTipoAtto);
		return mapNotNull(siacDAttoAmmTipo, TipoAtto.class, BilMapId.SiacDAttoAmmTipo_TipoAtto);
	}
	...
	public ListaPaginata<AttoAmministrativo> ricercaSintetica(RicercaAtti ricercaAtti, ParametriPaginazione parametriPaginazione, ModelDetailEnum... modelDetails) {
	...
	public AttoAmministrativo findProvvedimentoByIdModelDetail(int uid, ModelDetailEnum... modelDetails) {
	...
	public AttoAmministrativo ricercaPuntuale(RicercaAtti ricercaAtti) {
		List<AttoAmministrativo> elencoAttiAmministrativi = ricerca(ricercaAtti);
		
		if (ricercaAtti.getUid() != null) {
			return CollectionUtil.getOneOnly(elencoAttiAmministrativi);
		}
		...
		if (ricercaAtti.getStrutturaAmministrativoContabile() != null) {
			try {
				...
				throw new SystemException(ErroreCore.ENTITA_NON_UNIVOCA.getErrore("atto amministrativo", 
						String.format("%s/%s/%s-%s", 
								ricercaAtti.getAnnoAtto(), ricercaAtti.getNumeroAtto(), 
								ricercaAtti.getStrutturaAmministrativoContabile().getCodice(),
								ricercaAtti.getStrutturaAmministrativoContabile().getTipoClassificatore().getCodice()
				)));
			}
		}
		
		try {
			return CollectionUtil.getOneOnly(CollectionUtil.filter(elencoAttiAmministrativi, new Filter<AttoAmministrativo>() {
				@Override
				public boolean isAcceptable(AttoAmministrativo source) {
					return source.getStrutturaAmmContabile() == null;
				}
			}));
		}
		catch (IllegalArgumentException e) {
			throw new SystemException(ErroreCore.ENTITA_NON_UNIVOCA.getErrore("atto amministrativo", 
					String.format("%s/%s/-", ricercaAtti.getAnnoAtto(), ricercaAtti.getNumeroAtto())));
		}
	}
