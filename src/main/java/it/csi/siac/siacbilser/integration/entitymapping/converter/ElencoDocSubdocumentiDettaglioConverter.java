...
@Component
public class ElencoDocSubdocumentiDettaglioConverter extends ExtendedDozerConverter<ElencoDocumentiAllegato, SiacTElencoDoc> {
	
	/**
	 * Instantiates a new ElencoDocumentiAllegato attr converter.
	 */
	public ElencoDocSubdocumentiDettaglioConverter() {
		...
		if(src.getSiacRElencoDocSubdocs()!=null){
			for(SiacRElencoDocSubdoc sreds : src.getSiacRElencoDocSubdocs()){
				...
				if(SiacDDocFamTipoEnum.Spesa.equals(tipo) || SiacDDocFamTipoEnum.IvaSpesa.equals(tipo)){				
					...
					map(siacTSubdoc, s, BilMapId.SiacTSubdoc_SubdocumentoSpesa_Allegato);	
					subdocumenti.add(s);
					...
					map(siacTSubdoc, s, BilMapId.SiacTSubdoc_SubdocumentoEntrata_Allegato);	
					subdocumenti.add(s);
				}							
