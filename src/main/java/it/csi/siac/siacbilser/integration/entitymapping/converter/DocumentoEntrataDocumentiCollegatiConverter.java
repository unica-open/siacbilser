...
/**
 * The Class DocumentoEntrataDocumentiCollegatiConverter.
 */
@Component
public class DocumentoEntrataDocumentiCollegatiConverter extends ExtendedDozerConverter<DocumentoEntrata, SiacTDoc > {
	...
	/**
	 * Instantiates a new documento entrata documenti collegati converter.
	 */
	public DocumentoEntrataDocumentiCollegatiConverter() {
		...
		if(siacTDoc.getSiacRDocsPadre()!=null){
			for(SiacRDoc r: siacTDoc.getSiacRDocsPadre()){
				if(r.getDataCancellazione()==null) {
					...
					log.debug(methodName, "siacTDocFiglio.uid " + siacTDocFiglio.getUid());
					...
					if(SiacDDocFamTipoEnum.Entrata.getCodice().equals(docFamTipoCode)) {
						...
						docEntrata.setTipoRelazione(tipoRelazione); //TipoRelazione.fromCodice(r.getSiacDRelazTipo().getRelazTipoCode()));
						docEntrata.setImportoDaDedurreSuFattura(r.getDocImportoDaDedurre());
						...
						docSpesa.setTipoRelazione(tipoRelazione); //TipoRelazione.fromCodice(r.getSiacDRelazTipo().getRelazTipoCode()));
						docSpesa.setImportoDaDedurreSuFattura(r.getDocImportoDaDedurre());
