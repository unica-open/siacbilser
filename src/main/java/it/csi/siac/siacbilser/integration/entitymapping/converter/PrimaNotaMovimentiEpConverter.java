 ...
 */
@Component
public class PrimaNotaMovimentiEpConverter extends ExtendedDozerConverter<PrimaNota, SiacTPrimaNota> {
	

	/**
	 * Instantiates a new subdocumento Entrata subdocumento iva converter.
	 */
	public PrimaNotaMovimentiEpConverter() {
	...
	public PrimaNota convertFrom(SiacTPrimaNota src, PrimaNota dest) {
		...
		for(SiacTMovEp siacTMovEp: src.getSiacTMovEps()){
			if(siacTMovEp.getDataCancellazione() == null){
				MovimentoEP movimentoEp = map(siacTMovEp ,MovimentoEP.class, GenMapId.SiacTMovEp_MovimentoEP);
