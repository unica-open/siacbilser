...
public class DatiOperazioneUtil {
	...
	public final static <T extends SiacTEnteBase, E extends JpaRepository> List<T> bonificaDataCancellazione(List<T> entities, E repository,DatiOperazioneDto datiOperazione){
		if(!StringUtilsFin.isEmpty(entities)){
	...
	public final static <T extends SiacTBase> boolean hannoLoStessoUid(T o1, T o2){
		if(!CommonUtil.entrambiDiversiDaNull(o1, o2)){
			return false;
		}
	...
	public final static <T extends SiacTBase> T getValidoDataCancellazione(List<T> lista){
		for (T t : lista) {
			if(t.getDataCancellazione() == null) {
				return t;
			}
	...
	public final static <T extends SiacTBase> boolean tuttiPresentiNellAltraLista(List<T> o1, List<T> o2){
		if(o1 != null && o1.size()>0 && o2 != null && o2.size()>0){
			for(T it : o1){
				T finded = DatiOperazioneUtil.getById(o2, it.getUid());
				if(finded==null){
					return false;
				}
			}
			for(T it : o2){
				T dentroListaNew = DatiOperazioneUtil.getById(o1, it.getUid());
