...
public class InserisceClasseSoggettoService extends AbstractBaseService<InserisceClasse, InserisceClasseResponse>
{
	...
	@Override
	@Transactional
	public InserisceClasseResponse executeService(InserisceClasse serviceRequest)
	{
	...
	public void execute()
	{
		...
		this.res = new InserisceClasseResponse();
		try {
			// FIXME verificare che l'ente indicato nella richiesta sia quello del richiedente ?
			List<Errore>	errori = new ArrayList<Errore>();
			soggettoDad.inserisciClasseSoggetto(req.getClasseSoggetto(), req.getEnte(), req.getRichiedente(), errori);
