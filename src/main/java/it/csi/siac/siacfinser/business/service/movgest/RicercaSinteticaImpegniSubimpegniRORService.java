...
public class RicercaSinteticaImpegniSubimpegniRORService extends AbstractBaseService< RicercaSinteticaImpegniSubImpegni, RicercaSinteticaImpegniSubimpegniRORResponse>{
	...
	public RicercaSinteticaImpegniSubimpegniRORResponse executeService(RicercaSinteticaImpegniSubImpegni serviceRequest) {
...
//		Integer totImpegniTrovatiOLD = impegnoOttimizzatoDad.getCountRicercaImpegniSubImpegni(req.getEnte(), req.getRichiedente(), paramRic, false);
		Integer totImpegniTrovati = impegnoOttimizzatoDad.getCountRicercaImpegniSubImpegniROR(req.getEnte(), req.getRichiedente(), paramRic);
		
		
		List<SubImpegno> listaSubImpegni = new ArrayList<SubImpegno>();
		
		if(totImpegniTrovati!=null && totImpegniTrovati > 0){
			listaSubImpegni = impegnoOttimizzatoDad.ricercaSinteticaImpegniSubImpegniROR(req.getEnte(), req.getRichiedente(), 
					paramRic, numPagina, numRisPerPagina);
		...
		res.setListaSubImpegni(listaSubImpegni);
