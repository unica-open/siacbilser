...
public class RicercaAccertamentoPerChiaveOttimizzatoRORService extends RicercaAttributiMovimentoGestioneOttimizzatoService<RicercaAccertamentoPerChiaveOttimizzatoROR, RicercaAccertamentoPerChiaveOttimizzatoResponse> {
	...
	public RicercaAccertamentoPerChiaveOttimizzatoResponse executeService(RicercaAccertamentoPerChiaveOttimizzatoROR serviceRequest) {
	...
	public void execute() {
		...
		SubAccertamento subacc = accertamentoOttimizzatoDad.campiRiepilogoAccROR(annoEsercizio, req.getUidMovGest(),ente.getUid());
		if(subacc != null && esitoRicerca.getMovimentoGestione()!= null){
			esitoRicerca.getMovimentoGestione().setImportoDaRiaccertare(subacc.getImportoDaRiaccertare());
			esitoRicerca.getMovimentoGestione().setImportoMaxDaRiaccertare(subacc.getImportoMaxDaRiaccertare());
			esitoRicerca.getMovimentoGestione().setImportoModifiche(subacc.getImportoModifiche());
			esitoRicerca.getMovimentoGestione().setDifferitoN(subacc.getDifferitoN());
			esitoRicerca.getMovimentoGestione().setDifferitoN1(subacc.getDifferitoN1());
			esitoRicerca.getMovimentoGestione().setDifferitoN2(subacc.getDifferitoN2());
			esitoRicerca.getMovimentoGestione().setDifferitoNP(subacc.getDifferitoNP());
			esitoRicerca.getMovimentoGestione().setDaCancellareInesigibilita(subacc.getDaCancellareInesigibilita());
			esitoRicerca.getMovimentoGestione().setDaCancellareInsussistenza(subacc.getDaCancellareInsussistenza());
			esitoRicerca.getMovimentoGestione().setResiduoDaMantenere(subacc.getResiduoDaMantenere());
			esitoRicerca.getMovimentoGestione().setNumeroTotaleModifcheMovimento(subacc.getNumeroTotaleModifcheMovimento());
			//SIAC-7551
			esitoRicerca.getMovimentoGestione().setIncassatoAnnoSuccessivo(subacc.getIncassatoAnnoSuccessivo());
			esitoRicerca.getMovimentoGestione().setDocumentiNoIncassatiAnnoSuccessivo(subacc.getDocumentiNoIncassatiAnnoSuccessivo());
