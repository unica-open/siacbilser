...
public class SingleThreadCookieManager extends CookieHandler {
	
	private final ThreadLocal<CookieStore> cookieJars = new ThreadLocal<CookieStore>() {
	...
	public Map<String, List<String>> get(URI uri, Map<String, List<String>> requestHeaders) throws IOException {
		...
		CookieStore cookieJar = cookieJars.get();
		...
		for (HttpCookie cookie : cookieJar.get(uri)) {
			// apply path-matches rule (RFC 2965 sec. 3.3.4) and check for the possible "secure" tag (i.e. don't send 'secure' cookies over unsecure links)
				...
				try {
					...
					for (HttpCookie cookie : cookies) {
						...
						if (ports != null) {
							...
							if (ports.isEmpty()) {
								// Empty port list means this should be restricted to the incoming URI port
								cookie.setPortlist("" + port );
								if (shouldAcceptInternal(uri, cookie)) {
									cookieJar.add(uri, cookie);
								}
							} else {
								// Only store cookies with a port list IF the URI port is in that list, as per RFC 2965 section 3.3.2
					...
					// invalid set-cookie header string: no-op
				}
			}
		}
	}
	 ...
	 * @param cookies the cookies to sort
	 */
	...
	private static class CookiePathComparator implements Comparator<HttpCookie>, Serializable {
		/** For serialization */
		private static final long serialVersionUID = 7267901797506389937L;
